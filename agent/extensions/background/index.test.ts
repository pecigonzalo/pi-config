...
describe("background_run", () => {
	it("returns immediately and later delivers a completion message on success", async () => {
		...
		const run = h.tools.get("background_run");
		const result = await run.execute("call-1", { command: "true" }, undefined, undefined, h.ctx);
		...
		expect(h.entries.some((e) => e.customType === "background-job-resolved")).toBe(true);
	});

	it("delivers a failure message when the command exits non-zero", async () => {
		const h = createHarness();
		const run = h.tools.get("background_run");
		await run.execute("call-2", { command: "exit 3" }, undefined, undefined, h.ctx);
...
describe("background_status", () => {
	it("lists a job while running and reflects its resolved status afterward", async () => {
		...
		const started = await run.execute("call-3", { command: "true" }, undefined, undefined, h.ctx);
		const id = started.details.id as string;

		const immediate = await status.execute("call-4", {}, undefined, undefined, h.ctx);
		expect(immediate.details.jobs.some((job: { id: string }) => job.id === id)).toBe(true);

		await waitUntil(() => h.sentMessages.length > 0);
		const after = await status.execute("call-5", { id }, undefined, undefined, h.ctx);
	...
	it("reports not found for an unknown id", async () => {
		const h = createHarness();
		const status = h.tools.get("background_status");
		const result = await status.execute("call-6", { id: "nope" }, undefined, undefined, h.ctx);
...
describe("background_cancel", () => {
	it("cancels a running job and suppresses the completion message", async () => {
		const h = createHarness();
		const run = h.tools.get("background_run");
		const cancel = h.tools.get("background_cancel");

		const started = await run.execute("call-7", { command: "sleep 5" }, undefined, undefined, h.ctx);
		const id = started.details.id as string;

		const cancelResult = await cancel.execute("call-8", { id }, undefined, undefined, h.ctx);
		...
		expect(
			h.entries.some(
				(e) =>
					...
					(e.data as { id: string; status: string }).status === "cancelled",
			),
		).toBe(true);
	});
	...
	it("reports not found for an unknown id", async () => {
		const h = createHarness();
		const cancel = h.tools.get("background_cancel");
		const result = await cancel.execute("call-9", { id: "nope" }, undefined, undefined, h.ctx);
		expect(result.details.found).toBe(false);
	});
});
	...
	it("marks a job that never resolved before shutdown as unknown on resume", () => {
		...
		expect(
			h.entries.some(
				(e) =>
					e.customType === "background-job-resolved" &&
					(e.data as { id: string; status: string }).id === "orphan" &&
					(e.data as { id: string; status: string }).status === "unknown",
			),
		).toBe(true);
	});
});
