...
export const footerLayoutNameSchema = z.enum(FOOTER_LAYOUT_NAMES).describe("Active footer layout name.");
...
export const footerStarshipConfigFileSchema = z.object({
	...
	command: z.string().trim().min(1).describe("Command used to invoke Starship.").optional().catch(undefined),
	timeoutMs: z
		.number()
		.int()
		.positive()
		...
		.optional()
		.catch(undefined),
	shell: z.string().trim().min(1).describe("Value passed as STARSHIP_SHELL.").optional().catch(undefined),
});
