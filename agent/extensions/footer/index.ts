...
import { createSessionContextController, parseContextPreset } from "./session-context";
...
export default function footer(pi: ExtensionAPI) {
  ...
  for (const item of createStarshipItems(starship, config)) footerManager.registerItem(item);
  for (const item of createSessionContextItems(sessionContext, config, starship)) footerManager.registerItem(item);
  ...
  pi.registerCommand("widgets", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!preset) {
        ...
        if (!requested) {
          ctx.ui.notify(`Preset: ${current}  (options: ${footerLayoutNames.join(" · ")})`, "info");
        } else {
          ctx.ui.notify(`Unknown preset \"${requested}\". Options: ${footerLayoutNames.join(" · ")}`, "warning");
        }
        return;
      }
