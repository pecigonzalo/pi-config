...
function rainbow(text: string): string {
	...
	for (const char of text) {
		rendered +=
			char === " " || char === ":" || char === "." || char === "/"
				? char
				: hexFg(rainbowColors[colorIndex++ % rainbowColors.length] ?? modelColor, char);
	}
			...
			switch (segment) {
				...
				case "git": {
					...
					if (indicators.length > 0)
						rendered += ` ${theme.fg("dim", "[")}${indicators.join("")}${theme.fg("dim", "]")}`;
					return rendered;
				}
				...
				case "context": {
					...
					const ctxPct =
						usage?.tokens != null && ctxMax ? Math.min(100, (usage.tokens / ctxMax) * 100) : null;
