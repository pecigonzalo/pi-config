...
import { isTreeSitterAvailable, parseBashCommand, type ParsedBash, type ParsedCommand } from "./shell-parse";
import {
...
import { runSandboxedCommandAfterHealthCheck, SandboxHealthMonitor } from "./sandbox-lifecycle";
...
export default function (pi: ExtensionAPI, dependencies: PermissionsExtensionDependencies = {}) {
	...
	let ensureSandboxHealthyAfterIdle: (
		...
		async execute(id, params, signal, onUpdate, ctx) {
			...
			return runSandboxedCommandAfterHealthCheck({
				...
				execute: () => {
					...
					if (!currentActive || !currentRuntime || sandboxLifecycleGeneration !== lifecycleGeneration) {
						throw new Error(
							"Bash sandbox changed or became unavailable before command execution; blocked local fallback",
						);
					}
	...
	const loadApprovals = (ctx?: ExtensionContext) => {
		...
		const loaded = extractApprovalRecords(parsed, (message) => warnPermissionIssue(ctx, message), approvalsFile);
	...
	const saveApprovals = (ctx?: ExtensionContext): boolean => {
		...
		try {
			...
			warnPermissionIssue(
				ctx,
				`Failed to save approvals at ${approvalsFile}: ${error instanceof Error ? error.message : String(error)}`,
			);
			return false;
		}
	};
	...
	async function resetSandboxRuntime(ctx?: ExtensionContext, reason?: string) {
		...
		try {
			...
			if (ctx) {
				...
				warnPermissionIssue(
					ctx,
					`Sandbox reset failed${suffix}: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
		}
	}
	...
	async function initializeSandbox(ctx: ExtensionContext) {
		...
		try {
			...
			if (ctx.hasUI) {
				ctx.ui.notify(
					"Bash sandbox unavailable: install dependencies in ~/.pi/agent/extensions/permissions/",
					"warning",
				);
			}
			return;
		...
		try {
			...
			await sandboxRuntime.initialize(execution.config, execution.configKey, {
				onResetError: (err) =>
					warnPermissionIssue(
						ctx,
						`Sandbox reset failed before reinitializing: ${err instanceof Error ? err.message : String(err)}`,
					),
			});
	...
	async function runSandboxProbe(
		...
		if (!cwdWriteExpected) {
			const reason = policyAllowsCwdWrites
				? "sandbox config does not allow writes"
				: "active policy does not allow writes";
			return {
				ok: true,
	...
	async function repairSandboxRuntime(
		ctx: ExtensionContext,
		options: { reloadConfig?: boolean; signal?: AbortSignal } = {},
	): Promise<SandboxProbeResult> {
		throwIfAborted(options.signal);
	...
	ensureSandboxHealthyAfterIdle = async (
		ctx: ExtensionContext,
		execution: SandboxExecution,
		runtime: SandboxRuntimeAdapter,
		signal?: AbortSignal,
		...
		ctx.ui.notify(
			`Bash sandbox health probe failed after ${Math.round(idleMs / 1000)}s idle; repairing sandbox runtime. ${probe.message}`,
			"warning",
		);
	...
	async function promptForBlockReason(ctx: ExtensionContext): Promise<string | undefined> {
		const message = await ctx.ui.input("Reject permission request", "Optional: tell the agent what to do instead");
	...
	async function askPermission(
		...
		if (toolName === "bash") {
			...
			const uniquePrefixCandidates = parsedFocusCommand
				...
				: dedupeStrings(
						[approvalTarget.trim().split(/\s+/)[0]].filter((value): value is string => Boolean(value)),
					);
			...
			for (const candidate of uniquePrefixCandidates) {
				...
				bashLines.push(
					"",
					"Reusable prefix:",
					indentPermissionPreview(
						formatPermissionPreview(prefix, { maxLines: 4, maxChars: 240, preserveEnd: true }),
					),
				);
			...
			if (selectedPrefix) {
				...
				if (isPermanent) {
					...
					if (saveApprovals(ctx)) {
						ctx.ui.notify(
							`✓ Bash prefix saved permanently: bash-prefix:${selectedPrefix} (matches: ${selectedPrefix} *)`,
							"info",
						);
					} else {
						persistentApprovals = previousApprovals;
					...
					ctx.ui.notify(
						`✓ Bash session rule added: bash-prefix:${selectedPrefix} (matches: ${selectedPrefix} *)`,
						"info",
					);
				}
			}
	...
	async function applyExternalPathPolicy(
		...
		const lines = [`Tool:    ${toolName}`, `Profile: ${agentName}`, "Paths:", ...shown.map((p) => `  ${p}`)];
		if (more) lines.push(more.trimStart());

		const approvalTargets =
			externalPaths.length === 1 && externalPaths[0] !== undefined
		...
		if (
			choice === allowPathPermanentLabel &&
			savePathApprovals(
				toolName,
				approvalTargets ? [approvalTargets.targetPath] : externalPaths,
				projectRoot,
	...
	async function checkBashPermission(
		...
		if (fallbackMode === "ask-all-bash") {
			return askPermission(
				"bash",
				input,
				"Sandbox unavailable: confirmation required for all bash commands",
				projectRoot,
				ctx,
			);
		}
	...
	async function getUserBashOperations(ctx: ExtensionContext): Promise<BashOperations | undefined> {
		...
		return {
			exec: async (command, cwd, options) => {
				...
				return runSandboxedCommandAfterHealthCheck({
					healthMonitor: sandboxHealthMonitor,
					ensureHealthy: (signal) =>
						ensureSandboxHealthyAfterIdle(ctx, healthExecution, healthRuntime, signal),
			...
			if (rule.action === "allow") {
				...
				if (externalPolicy !== "allow" && externalPaths.length > 0) {
					...
					if (
						...
						return applyExternalPathPolicy(
							externalPolicy,
							toolName,
							input,
							externalPaths,
	...
	pi.registerCommand("permissions", {
		...
		getArgumentCompletions: (prefix) => PERMISSIONS_COMPLETIONS.filter((s) => s.value.startsWith(prefix.trim())),
		handler: async (args, ctx) => {
			...
			if (normalizedArgs === "sandbox disable" || normalizedArgs === "sandbox off") {
				...
				ctx.ui.notify(
					`Bash sandbox disabled for this session; bash exec mode: ${sandboxBashExecutionMode()}`,
					"warning",
				);
				return;
			}
			...
			if (normalizedArgs === "sandbox enable" || normalizedArgs === "sandbox on") {
				...
				ctx.ui.notify(
					`Bash sandbox: ${sandboxStatusText()}; bash exec mode: ${sandboxBashExecutionMode()}`,
					sandboxActive ? "info" : "warning",
				);
				return;
			}
			...
			const sampleRules = rules
				.slice(0, 5)
				.map((r) => `[${r.action}] ${r.tool}${r.match ? ` ${formatRuleMatch(r)}` : ""}`);
			...
			await ctx.ui.custom<void>((_tui, theme, _kb, done) => {
				...
				if (!verbose) {
					...
					lines.push(
						`  ${theme.fg("muted", "External path:")}${theme.fg(epColor, ` ${externalPath}`)}${theme.fg("dim", " (structured tools)")}`,
					);
					lines.push(
						`  ${theme.fg("muted", "Bash sandbox: ")}${theme.fg(sandboxActive ? "success" : "dim", sandboxStatus)}`,
					);
					lines.push(
					...
					if (sessionAllows.size > 0) {
						lines.push(
							`  ${theme.fg("muted", "Session tools:")}${theme.fg("dim", ` ${[...sessionAllows].join(", ")}`)}`,
						);
					}
					lines.push("");
					lines.push(
						`  ${theme.fg("muted", "Rules:        ")}${theme.fg("text", `${rules.length} total`)} ${theme.fg("success", `allow=${actionCounts.allow}`)} ${theme.fg("warning", `ask=${actionCounts.ask}`)} ${theme.fg("error", `block=${actionCounts.block}`)}`,
					);
					...
					lines.push("");
					lines.push(
						`  ${theme.fg("muted", "Protected:    ")}${theme.fg("warning", `read=${protectedResources.denyRead.length} write=${protectedResources.denyWrite.length}`)}`,
					);
					lines.push(
						`  ${theme.fg("muted", "More:         ")}${theme.fg("dim", "/permissions verbose  •  /permissions approvals  •  /permissions reset  •  /permissions sandbox probe|repair|enable|disable")}`,
					);
					lines.push("");
					...
					if (sessionAllows.size > 0) {
						lines.push("");
						lines.push(
							`  ${theme.fg("warning", "Session tool allows:")} ${theme.fg("dim", [...sessionAllows].join(", "))}`,
						);
					}
					if (sessionBashApprovals.length > 0) {
						lines.push("");
						lines.push(
							`  ${theme.fg("warning", "Session bash approvals:")} ${theme.fg("dim", `${sessionBashApprovals.length}`)}`,
						);
					}
					...
					if (sessionPathApprovals.length > 0 || persistentApprovals.length > 0) {
						...
						if (persistentApprovals.length > 5) {
							lines.push(
								`  ${theme.fg("dim", `  ... ${persistentApprovals.length - 5} more saved approvals`)}`,
							);
						}
					}
					...
					lines.push(
						`  ${theme.fg("muted", "Protected read: ")}${theme.fg("warning", `${protectedResources.denyRead.length}`)}`,
					);
					...
					lines.push(
						`  ${theme.fg("muted", "Protected write:")}${theme.fg("warning", ` ${protectedResources.denyWrite.length}`)}`,
					);
					...
					lines.push(
						`  ${theme.fg("muted", "Protected built-ins:")}${theme.fg("accent", ` ${builtinsState}`)}`,
					);
					lines.push(
						`  ${theme.fg("muted", "Overrides:      ")}${theme.fg("dim", `+read=${(pr.addDenyRead ?? []).length} +write=${(pr.addDenyWrite ?? []).length} -read=${(pr.unprotectRead ?? []).length} -write=${(pr.unprotectWrite ?? []).length}`)}`,
					);
					if (sandboxConfig?.filesystem) {
						const fsCfg = sandboxConfig.filesystem;
						lines.push(
							`  ${theme.fg("muted", "  denyRead:     ")}${theme.fg("dim", (fsCfg.denyRead ?? []).join(", ") || "(none)")}`,
						);
						if ((fsCfg.allowRead ?? []).length > 0)
							lines.push(
								`  ${theme.fg("muted", "  allowRead:    ")}${theme.fg("dim", fsCfg.allowRead!.join(", "))}`,
							);
						lines.push(
							`  ${theme.fg("muted", "  allowWrite:   ")}${theme.fg("dim", (fsCfg.allowWrite ?? []).join(", ") || "(none)")}`,
						);
						lines.push(
							`  ${theme.fg("muted", "  denyWrite:    ")}${theme.fg("dim", (fsCfg.denyWrite ?? []).join(", ") || "(none)")}`,
						);
					}
					if (sandboxConfig?.network) {
						...
						const allowLabel = unrestricted
							? "* (unrestricted)"
							: (netCfg.allowedDomains ?? []).join(", ") || "(none)";
						const denyLabel = unrestricted ? "(none)" : (netCfg.deniedDomains ?? []).join(", ") || "(none)";
						lines.push(
							`  ${theme.fg("muted", "  network:      ")}${theme.fg("dim", `allow=${allowLabel} deny=${denyLabel}`)}`,
						);
					...
					if (rules.length === 0) {
						...
						const truncate = (s: string, max: number) =>
							max <= 1 ? s.slice(0, Math.max(0, max)) : s.length > max ? `${s.slice(0, max - 1)}…` : s;
						...
						lines.push(
							`  ${theme.fg("borderMuted", "─".repeat(toolW + matchW + actionW + extW + sandboxW + reasonW + 12))}`,
						);
						...
						for (const rule of rules) {
								...
								epa === "allow"
									...
									: epa === "block"
										...
										: epa === "ask"
											...
											: "dim";
							const sandboxColor =
								sandboxRaw === "off" ? "warning" : sandboxRaw === "on" ? "success" : "dim";
							const epaStr = theme.fg(epaColor, pad(truncate(epa, extW), extW + 2));
							const sandboxStr = theme.fg(
								sandboxColor,
								pad(truncate(sandboxRaw, sandboxW), sandboxW + 2),
							);
							const reason = theme.fg("dim", reasonRaw);
							lines.push(
								`  ${tool}${theme.fg("muted", pad(matchParts[0] ?? "", matchW + 2))}${action}${epaStr}${sandboxStr}${reason}`,
							);
							for (const continuation of matchParts.slice(1)) {
								...
								lines.push(
									`  ${theme.fg("dim", emptyTool)}${theme.fg("dim", pad(`↳ ${continuation}`, matchW + 2))}${theme.fg("dim", emptyAction)}${theme.fg("dim", emptyExt)}${theme.fg("dim", emptySandbox)}`,
								);
							}
						}
	...
	async function showPermissionApprovals(ctx: ExtensionContext) {
		...
		await ctx.ui.custom<void>((_tui, theme, _kb, done) => {
			...
			for (const approval of scopedSession.slice(0, 10))
				lines.push(`  ${theme.fg("dim", "  ↳ ")}${theme.fg("dim", format(approval))}`);
			if (scopedSession.length > 10)
				lines.push(`  ${theme.fg("dim", `  ... ${scopedSession.length - 10} more`)}`);
			lines.push("");
			lines.push(`  ${theme.fg("accent", `Saved approvals (${scopedSaved.length})`)}`);
			for (const approval of scopedSaved.slice(0, 10))
				lines.push(`  ${theme.fg("dim", "  ↳ ")}${theme.fg("dim", format(approval))}`);
	...
	function resetPermissions(args: string | undefined, ctx: ExtensionContext) {
		...
		if (resetProject) {
			sessionPathApprovals.splice(
				0,
				sessionPathApprovals.length,
				...sessionPathApprovals.filter((a) => a.projectRoot !== projectRoot),
			);
			sessionBashApprovals.splice(
				0,
				sessionBashApprovals.length,
				...sessionBashApprovals.filter((a) => a.projectRoot !== projectRoot),
			);
		...
		if (resetAgent) {
			sessionPathApprovals.splice(
				0,
				sessionPathApprovals.length,
				...sessionPathApprovals.filter((a) => a.agentName !== agentName),
			);
			sessionBashApprovals.splice(
				0,
				sessionBashApprovals.length,
				...sessionBashApprovals.filter((a) => a.agentName !== agentName),
			);
