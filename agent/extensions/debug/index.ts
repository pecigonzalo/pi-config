...
function parsePromptArgs(args: string): PromptRequest {
	...
	if (
		...
		normalizedSubcommand === "--help" ||
		normalizedSubcommand === "-h"
	) {
		return { mode, raw, includeTools, help: true };
	}

	const options = normalizedSubcommand === "prompt" ? rest : isPromptOption(normalizedSubcommand) ? words : undefined;
	if (!options) {
		return { mode, raw, includeTools, help: true };
	}
...
async function showPromptDocument(document: PromptDocument, ctx: ExtensionCommandContext): Promise<void> {
	...
	await ctx.ui.custom<void>((tui, theme, _keybindings, done) => new PromptOverlay(tui, theme, document, done), {
		overlay: true,
		overlayOptions: {
			width: "95%",
			maxHeight: "92%",
...
function registerDebugCommand(pi: ExtensionAPI, getLastSnapshot: () => PromptSnapshot | undefined): void {
	pi.registerCommand("debug", {
		...
		getArgumentCompletions: (prefix) => DEBUG_COMPLETIONS.filter((s) => s.value.startsWith(prefix.trim())),
