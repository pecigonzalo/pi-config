...
function applyCommand(
	args: string,
	options: CodeHintsOptions,
	reset: () => void,
	status: () => string,
): CodeHintsCommandResult {
...
function filterNewErrors(
	results: LspFileDiagnostics[],
	baseline: Map<string, Set<DiagnosticFingerprint>>,
): LspDiagnosticItem[] {
	return results.flatMap((result) => {
		const baselineForFile = baseline.get(result.file);
		if (!baselineForFile) return [];
		return result.diagnostics.filter(
			(item) => item.severity === "error" && !baselineForFile.has(diagnosticFingerprint(item)),
		);
	});
}
...
function formatReport(
	...
	const candidateHints =
		baseline.size > 0 || options.requireBaseline
	...
	const timedOut = results
		...
		.map((result) => relative(cwd, result.file) || result.file);
...
function shouldKeepInModelContext(message: unknown): boolean {
	...
	const details =
		record.details && typeof record.details === "object" ? (record.details as { audience?: unknown }) : undefined;
...
export default function codeHintsExtension(pi: ExtensionAPI) {
	...
	const flushCodeHints = async (ctx: ExtensionContext, reason: CodeHintsFlushReason) => {
		...
		const pendingBaselines = files
			.map((file) => baselinePromises.get(file))
		...
		flushInFlight = (async () => {
			...
			const results = await service
				.diagnostics(files, { severity: "error", timeoutMs: DEFAULT_TIMEOUT_MS, signal: ctx.signal })
				.catch(() => []);
			if (!active || generation !== currentGeneration || dirtyGeneration !== currentDirtyGeneration) return;
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		const promise = service
			.diagnostics([input.path], { severity: "error", timeoutMs: DEFAULT_TIMEOUT_MS, signal: ctx.signal })
			.then((results) => {
	...
	pi.registerCommand("code-hints", {
		...
		getArgumentCompletions: (prefix) => CODE_HINTS_COMPLETIONS.filter((s) => s.value.startsWith(prefix.trim())),
