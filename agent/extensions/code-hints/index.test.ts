...
describe("code-hints formatting", () => {
	...
	it("reports only diagnostics that are new relative to baseline", () => {
		...
		const report = __test__.formatReport("/repo", after, __test__.collectErrorFingerprints(before), {
...
describe("code-hints input tracking", () => {
	it("resets remediation guards only for idle user prompts", () => {
		...
		expect(__test__.shouldResetRemediationFollowUp({ source: "interactive", streamingBehavior: "steer" })).toBe(
			false,
		);
		expect(__test__.shouldResetRemediationFollowUp({ source: "interactive", streamingBehavior: "followUp" })).toBe(
			false,
		);
	});
});
