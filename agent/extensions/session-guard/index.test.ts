...
describe("session-guard safety hardening", () => {
	...
	it("validates checkpoint refs before any destructive git mutation", async () => {
		...
		expect(calls).toEqual(["rev-parse --show-toplevel", "rev-parse --verify --quiet missing-ref^{tree}"]);
	});
	...
	it("creates a safety stash before reset/clean", async () => {
		...
		const stashIndex = calls.findIndex((value) => value.startsWith("stash push --include-untracked --message"));
	...
	it("fails safe for auto-restore in no-UI mode when working tree is dirty", async () => {
		...
		try {
			...
			expect(calls).toEqual(["rev-parse --show-toplevel", "status --porcelain --untracked-files=all"]);
