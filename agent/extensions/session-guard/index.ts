...
function formatInvalidSettingWarning(key: string, actual: unknown, fallback: string): string {
	return `Invalid sessionGuard.${key} value (${JSON.stringify(actual)}); using "${fallback}".`;
}

function parseBooleanSetting(key: string, value: unknown, warnings: string[]): boolean | undefined {
	if (value === undefined) return undefined;
	if (typeof value === "boolean") return value;
	warnings.push(formatInvalidSettingWarning(key, value, "false"));
	return false;
}

function parseDirtyRepoPolicySetting(value: unknown, warnings: string[]): DirtyRepoPolicy | undefined {
	if (value === undefined) return undefined;
...
function sanitizeSettings(raw: unknown): LoadedSettings {
	...
	const settings: SessionGuardSettings = {
		confirmNewSession: parseBooleanSetting("confirmNewSession", raw.confirmNewSession, warnings),
		confirmSwitchSession: parseBooleanSetting("confirmSwitchSession", raw.confirmSwitchSession, warnings),
		...
		restoreCodeOnFork: parseRestoreModeSetting("restoreCodeOnFork", raw.restoreCodeOnFork, warnings),
		restoreCodeOnTree: parseRestoreModeSetting("restoreCodeOnTree", raw.restoreCodeOnTree, warnings),
	};
...
function readSessionGuardSettings(filePath: string): unknown {
	const parsed = readJsonFile(filePath) as { default?: { sessionGuard?: unknown } } | undefined;
...
function shouldCaptureCheckpointsForContext(settings: SessionGuardSettings, ctx: ExtensionContext): boolean {
...
function isStaleSessionError(error: unknown): boolean {
	return (
		...
		/ctx is stale|session replacement or reload|captured pi or command ctx/.test(error.message)
	);
}
...
async function captureCheckpoint(
	...
	const checkpoint = await createCheckpoint(pi, cwd, capture, uniqueTargetIds);
...
async function checkDirtyRepo(
	...
	const choice = await ctx.ui.select(`You have ${changedFiles} uncommitted ${noun}.\n${actionLabel} anyway?`, [
		"Yes, proceed anyway",
...
function appendSafetySnapshotHint(message: string, snapshot: SafetySnapshotResult): string {
...
async function validateCheckpointRef(
	...
	const verifyTarget = checkpoint.kind === "commit" ? `${checkpoint.ref}^{tree}` : `${checkpoint.ref}^{commit}`;
	const verify = await pi.exec("git", ["rev-parse", "--verify", "--quiet", verifyTarget], {
		cwd: repoRoot,
	});
	...
	if (checkpoint.kind === "stash") {
		const parents = await pi.exec("git", ["rev-list", "--parents", "-n", "1", checkpoint.ref], { cwd: repoRoot });
...
async function createSafetySnapshot(pi: ExtensionAPI, repoRoot: string): Promise<SafetySnapshotResult> {
	const status = await pi.exec("git", ["status", "--porcelain", "--untracked-files=all"], { cwd: repoRoot });
	...
	if (stash.code !== 0) {
		return {
			created: false,
			error: formatCommandFailure("git stash push --include-untracked", stash.stdout, stash.stderr, stash.code),
		};
	}
	...
	const refResult = await pi.exec("git", ["rev-parse", "--verify", "--quiet", "stash@{0}"], {
		cwd: repoRoot,
	});
...
async function restoreCheckpoint(
	...
	if (reset.code !== 0) {
		return {
			error: appendSafetySnapshotHint(
				formatCommandFailure("git reset --hard HEAD", reset.stdout, reset.stderr, reset.code),
				safetySnapshot,
			),
	...
	if (clean.code !== 0) {
		return {
			error: appendSafetySnapshotHint(
				formatCommandFailure("git clean -fd", clean.stdout, clean.stderr, clean.code),
				safetySnapshot,
			),
			safetySnapshotCreated: safetySnapshot.created,
			safetySnapshotRef: safetySnapshot.ref,
	...
	if (checkpoint.kind === "commit") {
		const restore = await pi.exec(
			"git",
			["restore", `--source=${checkpoint.ref}`, "--staged", "--worktree", "--", "."],
			{
				cwd: repoRoot,
			},
		);
...
async function maybeRestoreCodeState(
	...
	const dirtyConfirmation = await confirmDirtyRestoreAllowed(pi, ctx, actionLabel);
	...
	if (restoreResult.error) {
		if (ctx.hasUI) ctx.ui.notify(`Code restore failed: ${restoreResult.error}`, "error");
		return { cancel: true };
	}
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_before_switch", async (event, ctx) => {
		...
		if (event.reason === "new" && settings.confirmNewSession) {
			const ok = await ctx.ui.confirm("Clear session?", "This will delete all messages in the current session.");
			if (!ok) {
