...
export interface WorkerSessionSpec {
	...
	/** Resolved model object for the worker, or undefined to use the session default. */
	model?: Model<any>;
...
/** Releases a worker's depth entry once its session finishes, so the registry doesn't grow unbounded. */
export function clearSubagentDepth(sessionId: string | undefined): void {
	if (sessionId) subagentDepthBySessionId.delete(sessionId);
}
