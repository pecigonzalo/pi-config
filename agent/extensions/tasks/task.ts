...
async function runSingleAgentViaAgentSession(
	...
	runId: string,
	parentDepth: number,
	parentCtx: Pick<ExtensionContext, "ui" | "hasUI" | "modelRegistry" | "model">,
	agentDir: string,
	...
	let workerModel: Model<any> | undefined;
	if (agentModel) {
		const parsed = parseAgentModelSpec(agentModel, parentCtx.model);
		if (!parsed) {
			return baseResult({ exitCode: 1, stderr: `Could not resolve model for task worker: ${agentModel}.` });
		}
		workerModel = parentCtx.modelRegistry.find(parsed.provider, parsed.modelId) as Model<any> | undefined;
		if (!workerModel) {
			return baseResult({
				exitCode: 1,
				stderr: `Model not found for task worker: ${parsed.provider}/${parsed.modelId}.`,
			});
		}
	}
	...
	try {
		...
		clearSubagentDepth(session.sessionManager.getSessionId());
...
async function openTaskRunSession(
	...
	// A still-running worker opens as the live attach view (subscribe-only overlay that never
	// session-replaces), never as a normal session replace -- replacing the current session
	// would dispose the delegating parent. Only a finished worker is safe to open as a real
	// persisted session.
	const liveControllerKey = makeTaskRunStepKey(run.runId, targetStep.step ?? preferredStep?.step ?? 0);
	if (isLiveController(getLiveTaskController(liveControllerKey))) {
		const attachResult = await attachTaskRunInTerminal(ctx as any, run, preferredStep);
		return {
			ok: attachResult.ok,
			opened: attachResult.ok,
			level: attachResult.level,
	...
	const tasksCommand = {
		...
		handler: async (args, ctx) => {
			try {
				...
				// here. `open` deliberately does not: a still-running step opens as the live attach
				// view (which must work *while* the task is running), and only the finished-
