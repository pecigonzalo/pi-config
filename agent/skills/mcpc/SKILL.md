...
In this Pi environment, invoke mcpc as `bunx @apify/mcpc` unless a global `mcpc` binary is installed. We should also use `bunx @apify/mcpc connect '$HOME/.mcp.json:<mcp-server>'` to connect to pre-configured MCP servers.
